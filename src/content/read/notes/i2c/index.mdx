...
> Note that in contrast to the "Multiple Reads", in a string of `WRITE`s, the target is the setter of the `ACK/NACK` bit, and it does _not_ terminate the srting of `WRITE`s with a `NACK`, it just `ACK`s and `STOP`s. 
